...
ui <- dashboardPage(
  dashboardHeader(
    title = span("Baloto Analyzer ", tags$small("Investigación Estadística")),
    titleWidth = 300
  ),
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      tags$footer(style = "padding: 10px; text-align: center;", tags$small("Versión 1.0 - © 2023"))
    ),
    width = 250
  ),
  ...
  dashboardBody(
    ...
    tabItems(
      # 1. BASE DE DATOS
      tabItem(tabName = "datos", fluidRow(
        box(
          title = "Conjunto de Datos Completos",
          width = 12,
          status = "primary",
          solidHeader = TRUE,
          helpText(
            "Datos históricos de todos los sorteos registrados. Filtre por fechas usando los controles superiores."
          ),
      ...
      tabItem(tabName = "distribuciones", fluidRow(
        box(
        ...
        box(
          title = "Medidas de Posición - Diagramas de Caja",
          width = 6,
          status = "info",
          plotlyOutput("boxplot_posiciones"),
          footer = "Distribución de valores numéricos por posición. Cada caja muestra: mínimo, Q1, mediana, Q3, máximo."
        )
      ), fluidRow(
        box(
      ...
      tabItem(tabName = "posicion", fluidRow(
        box(
          title = "Mapa de Calor por Posición",
          width = 6,
          status = "danger",
          solidHeader = TRUE,
        ...
        box(
          title = "Tabla de Contingencia Detallada",
          width = 8,
          status = "danger",
          solidHeader = TRUE,
        ...
        box(
          ...
          status = "danger",
          solidHeader = TRUE,
          downloadButton("descargar_posicion", "Exportar CSV", class = "btn-download"),
          br(),
          actionButton(
            "actualizar_analisis",
            "Actualizar Análisis",
      ...
      tabItem(tabName = "correlaciones", fluidRow(
        box(
      ...
      tabItem(tabName = "pruebas", fluidRow(
        box(
          title = "Pruebas de Uniformidad",
          width = 6,
          status = "warning",
      ...
      # 4.2 ALEATORIEDAD
      tabItem(tabName = "aleatoriedad", fluidRow(
        box(
          title = "Test de Rachas",
          width = 6,
          status = "warning",
